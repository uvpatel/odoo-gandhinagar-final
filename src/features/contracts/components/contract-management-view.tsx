...
import { ContractSubNav } from "@/components/contracts/contract-subnav";
...
const statusColors: Record<string, string> = {
  ...
  expiring: "border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-semibold",
};
...
export interface ContractManagementViewProps {
  initialStatusFilter?: string;
  initialViewMode?: "table" | "groups";
}
...
export function ContractManagementView({
  ...
  const [viewMode, setViewMode] = React.useState<"table" | "groups">(
    urlView === "groups" ? "groups" : initialViewMode
  );

  // Filters
  const [searchTerm, setSearchTerm] = React.useState("");
  const [statusFilter, setStatusFilter] = React.useState<string>(
    urlStatus || initialStatusFilter
  );
  const [departmentFilter, setDepartmentFilter] = React.useState<string>("all");
  const [employeeFilter, setEmployeeFilter] = React.useState<string | null>(urlEmployeeId);
  const [startDateFrom, setStartDateFrom] = React.useState<string>("");
  const [startDateTo, setStartDateTo] = React.useState<string>("");

  // Sync state when URL search parameters change dynamically
  React.useEffect(() => {
    if (urlStatus) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {viewMode === "groups"
              ? "Department and employee grouped breakdown of contract lifecycles and wage progression."
              : statusFilter === "active"
              ? "Displaying all currently active contracts applicable for payroll operations."
              : statusFilter === "expiring"
              ? "Contracts approaching expiration within the next 60 days requiring extension or renewal."
